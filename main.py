...
user_state = {}  # {chat_id: {'step': str, 'movie_name': str, 'selected_site': str, 'site_results': {site: {'titles': [], 'links': []}}, 'last_active': datetime}}
STATE_TIMEOUT = timedelta(minutes=30)
MAX_MESSAGE_LENGTH = 4000
MAX_RETRIES = 3
MAX_RESULTS_PER_SITE = 10  # Reduced for better UX
BUTTON_TEXT_LIMIT = 40  # Adjusted for cleaner buttons
MAX_LINKS_TO_DISPLAY = 5  # Limit displayed links
...
def create_movie_selection_keyboard(site, titles, offset=0):
    ...
    for i in range(offset, end_index):
        ...
        markup.add(InlineKeyboardButton(f"{site_info['emoji']} {title}", callback_data=f"select_{site}_{i}"))
    nav_buttons = []
    if offset > 0:
    ...
    markup.row(
        InlineKeyboardButton("🔙 Back", callback_data="back_to_site"),
    ...
    try:
        ...
        for title, link in zip(titles, links):
            if validate_url(link):
                ...
                valid_links.append((link, site))
        if valid_titles:
            logger.info(f"Fetched {len(valid_titles)} valid titles from {site} for query: {movie_name}")
            return {'titles': valid_titles, 'links': valid_links}
        return {}
    except Exception as e:
            ...
            if text.lower() == '/start':
                ...
                if state['step'] == 'awaiting_movie_name':
                    ...
                    try:
                        ...
                        send_long_message(
                            chat_id,
                            f"🌐 <b>Update domain for {site_keys[index].capitalize()}</b>\n\n"
                    ...
                    if update_site_domain(site_key, new_domain):
                        del user_state[chat_id]
                        sites = "\n".join([f"• {key.capitalize()}: <code>{SITE_CONFIG[key]}</code>" for key in SITE_CONFIG.keys()])
                        send_long_message(
                            chat_id,
                            f"✅ <b>Updated {site_key.capitalize()} to '{new_domain}'</b>\n\n"
                        ...
                        send_long_message(
                            ...
                            f"Invalid format for '{site_key}'. Domain must start with '{site_key}'.\n\n"
            ...
            if callback_data == 'cancel':
                ...
                if 'movie_name' in state:
                    ...
                    bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=message_id,
                        text=f"🎯 <b>Searching: {state['movie_name']}</b>\n\n🌐 <b>Select a site to search:</b>",
                        parse_mode='HTML',
                        reply_markup=create_site_selection_keyboard()
                    )
                    bot.answer_callback_query(callback['id'])
                ...
                bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_id,
                    text=f"🔍 <b>Searching '{state['movie_name']}' on {site_info['emoji']} {site_info['name']}...</b>\n\n⏳ <i>Please wait...</i>",
                    parse_mode='HTML'
                )
                results = search_movies_for_site(state['movie_name'], site)
                ...
                if not results:
                    bot.edit_message_text(
                        ...
                        message_id=message_id,
                        text=(
                            f"😔 <b>No results for '{state['movie_name']}'</b>\n\n"
                ...
                results_text = (
                    f"✨ <b>Found {len(titles)} results for '{state['movie_name']}'</b>\n\n"
                ...
                site_info = SITES.get(site, {'name': site.capitalize(), 'emoji': '🎬'})
                results_text = (
                    f"✨ <b>Results for '{state['movie_name']}'</b>\n\n"
                    f"🎬 <b>Site:</b> {site_info['emoji']} {site_info['name']}\n"
